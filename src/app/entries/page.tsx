...
import {
    ...
    AlertTriangle,
    ImageIcon,
    ImageIcon as DocumentationIcon
} from 'lucide-react';
                                            ...
                                            <td className="px-4 py-3">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-8 h-8 rounded-lg overflow-hidden bg-gray-50 flex-shrink-0 border border-gray-100">
                                                        {(entry['Documentation Image'] || (entry as any).imageUrl) ? (
                                                            <img src={entry['Documentation Image'] || (entry as any).imageUrl} alt="" className="w-full h-full object-cover" />
                                                        ) : (
                                                            <div className="w-full h-full flex items-center justify-center text-gray-200 font-bold text-[10px]">S</div>
                                                        )}
                                                    </div>
                                                    <div className="min-w-0">
                                                        <div className="font-bold text-gray-900 text-sm truncate">{entry['Code'] || (entry as any).code}</div>
                                                        <div className="text-[10px] text-gray-400 line-clamp-1">{entry['Specification Details'] || (entry as any).spec}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-4 py-3">
                                                <div className="text-xs font-bold text-gray-700 truncate">
                                                    {entry['Car Model'] || entry.carModel || (entry as any).carModelId?.name}
                                                </div>
                                                <div className="text-[10px] text-gray-400 truncate">
                                                    {entry['Variant'] || entry.variant || (entry as any).variantId?.name} • {entry['Part Name'] || entry.partName || (entry as any).partId?.name}
                                                </div>
                                            </td>
                                            <td className="px-4 py-3">
                                                <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50 text-blue-700 text-[9px] font-black uppercase rounded-md border border-blue-100">
                                                    {entry['Region'] || entry.region || (entry as any).regionId?.name}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3 text-right">
                                                <div className="flex items-center justify-end gap-1">
                                                    <button
                                                        ...
                                                        className="p-1.5 hover:bg-blue-100 text-gray-300 hover:text-blue-600 rounded-lg transition-all"
                                                    >
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4">
                        ...
                        >
                            <div className="relative h-48 sm:h-64 bg-gray-50 border-b border-gray-100">
                                {(selectedEntry['Documentation Image'] || (selectedEntry as any).imageUrl) ? (
                                    <img src={selectedEntry['Documentation Image'] || (selectedEntry as any).imageUrl} alt="" className="w-full h-full object-contain p-4" />
                                ) : (
                            ...
                            <div className="p-5 sm:p-8">
                                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
                                    <div>
                                        <div className="inline-block px-2.5 py-1 bg-gray-900 text-white text-[10px] font-black uppercase rounded-lg mb-2 tracking-widest shadow-lg shadow-gray-200">
                                            {selectedEntry['Code'] || (selectedEntry as any).code}
                                        </div>
                                        <h2 className="text-xl sm:text-2xl font-black text-gray-900 tracking-tight leading-tight">
                                            {selectedEntry['Car Model'] || (selectedEntry as any).carModel || (selectedEntry as any).carModelId?.name} <span className="text-blue-600 font-medium">{selectedEntry['Variant'] || (selectedEntry as any).variant || (selectedEntry as any).variantId?.name}</span>
                                        </h2>
                                    </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                                    <div className="p-4 bg-gray-50/50 rounded-2xl border border-gray-100">
                                        ...
                                        <div className="font-bold text-gray-800 text-sm">
                                            {selectedEntry['Part Name'] || (selectedEntry as any).partName || (selectedEntry as any).partId?.name}
                                        </div>
                                        <div className="text-[11px] text-gray-500 font-medium">
                                            {selectedEntry['Category'] || (selectedEntry as any).category || (selectedEntry as any).categoryId?.name}
                                        </div>
                                    </div>
                                    <div className="p-4 bg-gray-50/50 rounded-2xl border border-gray-100">
                                        <div className="text-[9px] font-bold text-gray-400 uppercase mb-1 tracking-widest">Regional Scope</div>
                                        <div className="font-bold text-gray-800 text-sm">
                                            {selectedEntry['Region'] || (selectedEntry as any).region || (selectedEntry as any).regionId?.name}
                                        </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="p-4 sm:p-5 bg-white border border-gray-100 rounded-2xl text-gray-700 text-xs sm:text-sm leading-relaxed whitespace-pre-wrap shadow-sm">
                                        {selectedEntry['Specification Details'] || (selectedEntry as any).spec}
                                    </div>
                                </div>
                            </div>
                        </motion.div>
